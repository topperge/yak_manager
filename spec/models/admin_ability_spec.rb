...
describe AdminAbility do
  ...
  it 'prevents a regular user from managing companies he does not belong to' do
    twitter = Company.create!(:name => 'Twitter' )
    ...
    normal_guy = User.create!(:company_id => twitter.id + 1, :password => 'passpass', :email=>'joe@gmail.com', :role => 'user')
  ...
  it 'lets a normal user read companies like through activeadmin' do
    livingsocial = Company.create!(:name => 'Living Social' )
    normal_guy = User.create!(:company_id => nil, :password => 'passpass', :email=>'joe@gmail.com', :role => 'user')
    non_admin = AdminAbility.new(normal_guy)
